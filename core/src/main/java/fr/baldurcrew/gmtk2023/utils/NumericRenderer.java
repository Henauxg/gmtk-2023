...
public class NumericRenderer implements Disposable {
    ...
    public void renderNumber(SpriteBatch spriteBatch, int number, Vector2 position, Vector2 size) {
        spriteBatch.draw(numbers[number % 10], position.x - size.x / 2f, position.y - size.y / 2f, size.x, size.y);
    }
}
